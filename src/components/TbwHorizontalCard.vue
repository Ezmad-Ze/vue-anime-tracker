<template>
    ...
    <div role="AnimeCard"
        ...
        " class="px-1 text-white selection:bg-yellow-700">
            <Icon data-testid="tbw" class="text-yellow-300 w-5 h-5" icon="material-symbols:bookmark"
                v-if="tbw?.tbr" />
            <Icon data-testid="nottbw" class="text-yellow-300 w-5 h-5" icon="material-symbols:bookmark-outline-rounded"
                v-else />
        </button>
    </div>
</template>
...
//emits
const emit: (event: "tbwHandler", ...args: any[]) => void = defineEmits([
    'tbwHandler',
])
